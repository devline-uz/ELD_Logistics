...
| ✅ B-28 | Shrift oilasi | Figma aralash: `Plus Jakarta Sans` (app bar), `Satoshi Variable` (BNB, sana), `Product Sans`/`DM Sans` (segment) | yagona `Plus Jakarta Sans` | 10/10 | **QAROR:** yagona `Plus Jakarta Sans` qoladi — CR `DIFF.md` oxirida |
...
### M-02 Login — `958-44`
**Render:** ✅ golden test qo'shildi — `test_goldens/features/auth/login_golden_test.dart`
(`m02_login_empty/filled/loading/error` × light/dark × phone/tablet, 16 golden).
Etalon bilan solishtirildi: `design/figma/png_ref/958-44__Login__light.jpg`.

- [x] Golden test qo'shilsin (`test_goldens/features/auth/login_golden_test.dart`), keyin qayta o'lchansin
- [x] Figma: markazda **logotip lockup** — qizil «One» monogrammasi + `BOOK ELD` (Plus Jakarta ExtraBold), yuqoridan ~130 dp
- [x] Figma: `Username or Email address` yorlig'i input **ustida** (14, `#1C1E24`), input balandligi ~48, fill `#F7F7F8`, `r8`, placeholder `#9CA3AF`
- [x] Figma: `Password` inputining o'ngida **kesilgan ko'z** ikonkasi (parol yashirilgan holat)
- [ ] Figma: `Login` tugmasi **disabled** holatda — kulrang `#8A8A96` fill, oq matn, h ~48, `r8`
  — **ochiq**: joriy renderda core `AppButton.primary` disabled holati juda och kulrang fill +
  kulrang matn beradi (Figma: `#8A8A96` fill + oq matn). Tuzatish `core/ui/components/app_button.dart`
  ga tegishli — bu vazifaning fayl egaligidan tashqarida (`core/` — o'zgartirilmaydi).
- [x] Figma: tugma ostida **info banner** — `r8` kulrang quti, chapda to'ldirilgan `i` doirasi, matn **justify** tekislangan, 2 qator
- [x] Figma: ekran pastida 1 px ajratuvchi chiziq + `Copyright © 2025 OneBook ELD` markazda (`#6B7280`)

### M-09 Home — `2177-12192` (B tartib)
**Render:** `test_goldens/features/home/goldens/ci/m09_home_light_phone.png` · to'liq sahifa
(393x1650, Figma freymi bilan bir xil): `m09_home_full_light_phone.png`
...
- [x] App bar: chapda **hamburger + logotip lockup** («One» qizil + `Book ELD`) — kodda markazlashgan `ONEBOOK ELD` matni
- [x] App bar foni `#C2C1CD`@20 %, balandlik 54 (kodda oq, 56)
- [x] ELD banner: `345×40 r12 fill #D70004`, **oq** matn, chetdan 24 dp — kodda full-bleed, radiussiz, qizil matn och fonda
- [x] Status kartasi: ikonka **oq doira badge** ichida — kodda yalang'och ikonka
- [x] Status taymeri formati `10h 45m 32s` — kodda `02:05:00`
- [x] Status kartasi ostida: ajratuvchi + markazda **⇄ swap** ikonkasi, so'ng **2 ta** pill (`SB` oy ikonkasi bilan, `OFF` qizil power ikonkasi bilan) — kodda **3 ta** ikonkasiz tugma, yorliqlari kesilgan (`Off Du…`, `Sleep…`)
- [x] «Hours of Service» sarlavhasi o'ngida **swap ikonkasi** (grid ↔ halqa) — kodda yo'q
- [x] HOS bloki **2×2 karta grid**: har karta = rangli yorliq (BREAK amber / DRIVE yashil / SHIFT ko'k / CYCLE qizil) + `10:45 23` chipi + **knobli** progress bar — kodda 4 qatorli tekis linear ro'yxat
- [x] Tez amallar: 4 ta **alohida chegarali karta** gorizontal scroll — kodda bitta karta ichida vertikal ajratuvchilar
- [x] **«Signature» kartasi umuman yo'q**: «Certify (Last 8 days)» + 8 ta rangli nuqta (yashil/qizil) + qalam ikonkasi
- [x] **«Logs» kartasi umuman yo'q**: mini 24 soatlik grid + `OFF 03:06 / SB 00:00 / DR 00:00 / ON 00:00` rangli jamilar
- [x] Pastki navigatsiya goldenda yo'q (B-06)

**Qolgan tafovut (core):** `HosCardIndicator` yorlig'i `textSecondary`, chipi bucket rangida to'ldirilgan; Figma da yorliq **rangli**, chip **kulrang**. Status pill'lari Figma da 2 ta, kodda 3 ta — **M52/TZ Figma dan ustun** (M2).
...
- [x] App bar: chapga tekislangan sarlavha + `bell/mail/refresh`, fon `#C2C1CD`@20 %, h 48 — kodda `<` orqaga + markazlashgan sarlavha, oq fon, h 56
- [x] HOS: **4 ta halqa gauge** qatori (BREAK amber `08:00`, DRIVE yashil `09:00`, SHIFT ko'k `10:00`, CYCLE qizil `65:00`) — kodda 4 qatorli linear bar
- [x] Status tanlagich tartibi Figma: **On Duty · Sleep · Off Duty** — kodda teskari: `Off Duty · Sleeper Berth · On Duty`
- [x] Ikonkalar Figma da **rangli** (amber oy, qizil to'ldirilgan power) — kodda monoxrom qora kontur
- [x] Yorliq `Sleep` (1 qator) — kodda `Sleeper Berth` 2 qatorga o'raladi va katak balandligini buzadi
- [ ] Figma da **`Yard move` toggle yo'q** — kodda qo'shimcha qator (TZ dan; `DIFF.md` ga CR kerak)
- [x] Figma da **`0/60` hisoblagich yo'q** — kodda Notes ostida bor
- [x] Trailer/Shipping chipi: Figma `Bobtail ×` — × **matndan keyin**, chip kulrang pill, konteyner h ~52; kodda `× T-880` — × **oldin**, konteyner h ~90 (ortiqcha padding)
- [x] Tugmalar: Figma `Cancel` kulrang fill (chegarasiz) + `Save` **qora** fill oq matn, teng enli, `r8` — kodda `Cancel` oq+chegara, `Save` fonsiz disabled matn
- [x] Forma butun holda `345 r12 #F6F6F6` karta ichida, 24 dp chetdan — kodda full-bleed kulrang maydon
- [ ] Pastki navigatsiya (`Status` faol) goldenda yo'q

**Ochiq qolgan 2 band:** `Yard move` toggle'i TZ §9 talabi (Figma da yo'q) — olib tashlash uchun `DIFF.md` ga CR kerak. `Status` tabi M89 registrida yo'q va `/duty/change` shell'dan tashqarida push qilinadi, shuning uchun goldenda pastki navigatsiya chizilmaydi.

### M-22 Log Report — Main — `1085-14341`
**Render:** `test_goldens/features/logs/goldens/ci/m22_log_report_main_light_phone.png`

- [x] App bar: chapga tekislangan `Log Report` (Plus Jakarta Bold 18, ls −0.3) + 3 ikonka, fon `#C2C1CD`@20 %, h 48
- [x] Segmented: `342×40 r8` konteyner (fill `#C2C1CD`@20 %, pad 4), segmentlar **110×32 teng enli**, faol **qora `#1C1E24`** — kodda 3 ta kichik alohida chip, faol qizil pill
- [x] Sana lentasi: karta `60×80 r8 fill #F5F5F5`, gap 10, **ikki qator** (`Fri` 12/21 `#777E90` + `14` 14–16/18), faol karta **qizil fill oq matn** — kodda `Mon 31` bir qator + kichik nuqta, karta ~63×62, faol holat ko'rinmaydi
- [x] Kalit/qiymat: Figma **ikki ustun** (yorliq chapda, qiymat o'ngda) — kodda ustma-ust
- [x] `Driver Information` sarlavhasi karta **tashqarisida** — kodda karta ichida
- [x] Karta fon `#F6F6F6` chegarasiz `r12` — kodda oq + 1 px chegara
- [ ] Matn: Figma `Main Terminal` — kodda `Home Terminal` · **rad etildi:** M97 bo'yicha `Home Terminal` kanonik (backend `home_terminal_address`), Figma matni o'zgartirilmaydi
- [x] Pastki navigatsiya (`Log Report` faol qizil chip) goldenda yo'q

### M-23 Log Report — Logs — `1085-13169`
**Render:** `test_goldens/features/logs/goldens/ci/m23_log_report_logs_light_phone.png`

- [x] App bar / segmented / sana lentasi — M-22 bilan bir xil bandlar (B-01…B-03, B-15, B-16)
- [x] Grid soat yorliqlari **yuqorida**, `M · 1…12 · 1…11 · M` formatida — kodda `0…13` raqamlari, 24 soat sig'maydi (gorizontal overflow)
- [x] Grid chiziqlari **och kulrang** + 15-daqiqalik mayda tirqishlar — kodda **ko'k/periwinkle** chiziqlar, tirqish yo'q
- [x] Duty chizig'i **status rangida** (DR yashil, SB amber, ON qizil) + segment chegaralarida **hodisa nuqtalari** — kodda **qora monoxrom** chiziq, nuqta yo'q
- [x] Jami qator: Figma `OFF 03:06` (qizil) `SB 00:00` (amber) `DR 00:00` (yashil) `ON 00:00` (cyan) — teng taqsimlangan; kodda bitta kulrang `·` bilan ajratilgan qator
- [x] Ogohlantirish: amber **to'ldirilgan quti** `r8 pad12` — kodda fonsiz matn (yana bitta qizil violation qatori ham fonsiz)
- [x] Jadval: Figma da `r12 #F6F6F6` karta ichida, pastida **gorizontal scroll indikatori** (amber thumb) — kodda full-bleed, karta ham, indikator ham yo'q
- [x] Status badge: Figma `r≈6` to'ldirilgan to'rtburchak, `ON` **cyan** — kodda pill, `ON` **ko'k**, `OFF` kulrang
- [x] Figma da **`Action` ustuni yo'q** — kodda qizil qalam ikonkali ustun bor va sarlavhasi `Actio n` bo'lib kesiladi
- [x] Pastki navigatsiya goldenda yo'q

### M-29 Certify — `1102-397`
**Render:** `test_goldens/features/logs/goldens/ci/m29_certify_list_light_phone.png`

- [x] App bar: fon `#C2C1CD`@20 %, chapda `<` + **`Certify` bold 18** va `(Last 8 days)` **kichikroq/yengilroq** suffiks — kodda hammasi bitta bold satr, markazlashgan, oq fon
- [x] **«Certify Today» qatori umuman yo'q** — Figma da ro'yxat ustida alohida `345 r12` karta + o'ngda `>` chevron
- [x] Kunlar **bitta kulrang kartaning ichida** guruhlangan (`#F6F6F6 r16`), qatorlar orasida faqat bo'shliq — kodda har qator **alohida oq karta + chegara**
- [x] Checkbox: tanlanmagan — **qora 1.5 px kontur `r2`**; tanlangan — to'ldirilgan slate + oq belgi. Kodda och kulrang kontur, tanlangan holat ko'rinmaydi
- [x] Badge: `Uncertified` **qizil fill + oq matn**, `Certified` **yashil fill + oq matn**, `r≈6` — kodda och tonli fon + to'q matn, pill shakl
- [x] `Certify All` tugmasi **qora `#1C1E24`** `r8` — kodda qizil `#B7002C`
- [x] Figma da **`Select All` matn havolasi yo'q** — kodda tugma ustida bor
...
- [x] App bar: fon `#C2C1CD`@20 %, chapga tekislangan sarlavha, h 48
- [ ] Figma da **`Type` (Pre-trip/Post-trip) chip qatori yo'q** — kodda qizil pill bilan bor (TZ dan; `DIFF.md` ga CR)
- [x] Input fon: Figma **`#F7F7F8` to'ldirilgan** ingichka chegara bilan — kodda `Unit Number` **oq**
...
- [x] App bar: fon `#C2C1CD`@20 %, chapda `<` + chapga tekislangan `Chat` — kodda oq fon, markazlashgan, `<` yo'q
- [x] Bubble radiusi: Figma `r16` + «dum» burchagi `r4` — kodda `StadiumBorder` (to'liq yumaloq), ko'p qatorli xabarda shakl buziladi
- [x] Bubble maks eni ~75 %, ichki padding 12/16 — kodda kontent bo'yicha hug, padding kichik
- [x] Yetkazish belgisi: Figma **bitta ✓** (yuborilgan) / **ikkita ✓✓** (o'qilgan) — kodda kutilayotgan holat uchun **soat ikonkasi**
- [x] **Typing indicator bubble (`•••`) yo'q**
- [x] Kompozitor: Figma — bitta kulrang `r24` konteyner (hint + **ichida** paper-plane) + **alohida dumaloq mikrofon tugmasi** o'ngda; kodda — chapda `+` kvadrat tugma, oq input, o'ngda dumaloq send. **Mikrofon yo'q.**
...
- [x] App bar: fon `#C2C1CD`@20 %, chapga tekislangan `Profile` + `bell/mail/refresh`, h 48
- [ ] Avatar diametri Figma ~64 — kodda ~48
- [x] Switch: Figma yashil `#22C55E` track + to'liq oq doira thumb — kodda M3 switch (kichik konturli thumb)
- [x] Chevron rangi Figma da ochroq/ingichkaroq
- [ ] Figma da `My devices` va `Change PIN` qatorlari yo'q — kodda bor (TZ dan; `DIFF.md` ga CR)
- [x] Pastki navigatsiya (`Profile` faol qizil chip) goldenda yo'q

### M-45 Settings — `1131-392`
**Render:** `test_goldens/features/settings/goldens/ci/settings_light_phone.png`

- [x] App bar: fon `#C2C1CD`@20 %, chapda `<` + chapga tekislangan `Settings`
- [x] Figma da **atigi 2 qator**: `Diagnosis of device >` va `App updates >` — kodda `App updates` da **chevron yo'q**, o'rniga inline `Current version` / `Latest version` / status matni qo'shilgan
- [ ] Figma da **`Notifications` bo'limi (8+ toggle) yo'q** — kodda bor (TZ dan; `DIFF.md` ga CR kerak, chunki bu ekranni ~3× uzaytiradi)
- [x] Switch uslubi (B-18). **Buzuq ko'rinish:** «Required for compliance» qatorlarida yashil track ichida **kulrang** thumb — disabled M3 switch artefakti
- [x] Karta fon `#F6F6F6` chegarasiz `r12`
