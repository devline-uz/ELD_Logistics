...
export function UnitFormModal({ open, onClose, unit }: UnitFormModalProps) {
  ...
  useEffect(() => {
    if (open) {
      form.reset(unit ? unitToFormValues(unit) : unitFormDefaultValues);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- faqat modal ochilganda qayta o'rnatiladi
  }, [open, unit]);
